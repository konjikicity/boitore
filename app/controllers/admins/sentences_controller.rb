class Admins::SentencesController < Admins::ApplicationController
  before_action :set_sentence, only: %i[edit update show destroy]

  def index
    @sentences = Sentence.all.page(params[:page]).per(8).order(:id)
  end
  ...
  def create
    ...
    if @sentence.save
      redirect_to admins_sentences_path,
  ...
  def update
    if @sentence.update(sentence_params)
      redirect_to admins_sentence_path(@sentence),
